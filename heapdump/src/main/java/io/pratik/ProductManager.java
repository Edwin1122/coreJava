...
public class ProductManager {
    private ProductGroup regularItemsList = new ProductGroup();

    private ProductGroup discountedItemsList = new ProductGroup();
    ...
    public void populateProducts() {
        ...
        for (int loop = 0; loop < 10; loop++) {
            if(loop % 2 == 0) {
                createObjects(regularItemsList, dummyArraySize);
            }else {
                createObjects(discountedItemsList, dummyArraySize);
            }
            System.out.println("Memory Consumed till now: " + loop + "::"+ regularItemsList + " "+ discountedItemsList);
